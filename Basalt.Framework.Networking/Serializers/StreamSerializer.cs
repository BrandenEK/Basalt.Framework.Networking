...
public class StreamSerializer : IMessageSerializer
{
    ...
    public byte[] Serialize(BasePacket packet)
    {
        ...
        stream.Write_ushort((ushort)data.Length);
        stream.Write_byte(info.PacketId);
    ...
    public IEnumerable<BasePacket> Deserialize(byte[] data)
    {
        ...
        {
            ushort length = stream.Read_ushort();
            byte type = stream.Read_byte();
            byte[] bytes = stream.Read_data(length);
